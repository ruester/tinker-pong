...
int lcd_20x4_write_line(LCD20x4 *lcd_20x4, uint8_t line, uint8_t position, char text[20]) {
	...
	wl.length = ipcon_leconvert_uint16_to(sizeof(WriteLine_));
	wl.line = line;
...
int lcd_20x4_clear_display(LCD20x4 *lcd_20x4) {
	...
	cd.length = ipcon_leconvert_uint16_to(sizeof(ClearDisplay_));
...
int lcd_20x4_backlight_on(LCD20x4 *lcd_20x4) {
	...
	bo.length = ipcon_leconvert_uint16_to(sizeof(BacklightOn_));
...
int lcd_20x4_backlight_off(LCD20x4 *lcd_20x4) {
	...
	bo.length = ipcon_leconvert_uint16_to(sizeof(BacklightOff_));
...
int lcd_20x4_set_config(LCD20x4 *lcd_20x4, bool cursor, bool blinking) {
	...
	sc.length = ipcon_leconvert_uint16_to(sizeof(SetConfig_));
	sc.cursor = cursor;
...
static int lcd_20x4_callback_wrapper_button_pressed(LCD20x4 *lcd_20x4, const unsigned char *buffer) {
	ButtonPressedCallback_ *bpc = (ButtonPressedCallback_ *)buffer;

	((button_pressed_func_t)lcd_20x4->registered_callbacks[bpc->function_id])(bpc->button);
	return sizeof(ButtonPressedCallback_);
}

static int lcd_20x4_callback_wrapper_button_released(LCD20x4 *lcd_20x4, const unsigned char *buffer) {
	ButtonReleasedCallback_ *brc = (ButtonReleasedCallback_ *)buffer;

	((button_released_func_t)lcd_20x4->registered_callbacks[brc->function_id])(brc->button);
	return sizeof(ButtonReleasedCallback_);
}

void lcd_20x4_register_callback(LCD20x4 *lcd_20x4, uint8_t id, void *callback) {
